...
export interface IStorageProps<T> {
  /** Storage key */
  key: string;
  ...
  /** If set to true, value will be updated is useEffect after mount */
...
function serializeJSON<T>(value: T, hookName: string) {
  try {
    ...
    throw new Error(`[sira] ${hookName}: Failed to serialize the value`);
  }
}
...
export function createStorage<T>(type: StorageType, hookName: string) {
  const eventName =
    type === "localStorage" ? "sira-local-storage" : "sira-session-storage";
...
export const useLocalStorage = <T = string>(props: IStorageProps<T>) => {
